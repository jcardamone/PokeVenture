...
async def instantiateItemList():
    with open('Venture Items - DatabaseItems.csv', 'r', encoding = "UTF-8") as file:
...
async def instantiatePkmnMoveList():
    with open('Pokeventure Moves - DatabaseMoves.csv', 'r', newline = '', encoding = "WINDOWS-1252") as infile:
...
async def pkmn_search(ctx, *, term):
    ...
    try:
        ...
        output += f' -- **Power**: {found[3]}\n'
        output += f'**Dmg Mods**: {(found[2] or "None")} + {(found[4] or "None")}\n'
...
async def instantiatePkmnStatList():
    with open('PokeVentureStats.csv', 'r', newline = '', encoding = "WINDOWS-1252") as infile:
        reader = csv.reader(infile)
        for row in reader:
...
async def pkmn_search(ctx, *, term):
    ...
    try:
        ...
        output += f'**Strength**: {found[5]})\n'
        output += f'**Dexterity**: {found[7]})\n'
        output += f'**Endurance**: {found[9]})\n'
        output += f'**Special**: {found[11]})\n'
        output += f'**Spirit**: {found[13]})\n'
        ...
        if found[16] != '': #hidden
            output += f' ({found[16]})'
        output += '\n'
        output += f'**Evolves From**: {(found[17] or "No")}\n'
        output += f'**Evolution Method**: {found[18]}\n'
	output += f'**Hieght Meters**: {found[19]}\n'
	output += f'**Weight KG**: {found[19]}\n'
...
async def instantiatePkmnLearnsList():
    with open('Venture MovePool - MovePool.csv', 'r', newline = '', encoding = "WINDOWS-1252") as infile:
        reader = csv.reader(infile)
        for row in reader:
